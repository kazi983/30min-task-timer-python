...
from widgets.drag_drop_listbox import DragDropListbox
from screens.improvement_request_dialog import ImprovementRequestDialog
...
class TaskManagerApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # 改修要望サービス・ダイアログ初期化
        MODE = os.getenv("TASK_MODE", "production")
        improvement_data_file = APP_DATA_DIR / (
            "improvement_requests_test.json" if MODE == "test" else "improvement_requests.json"
        )
        self.improvement_service = ImprovementRequestService(improvement_data_file)
        self.improvement_dialog = ImprovementRequestDialog(self.root, self.improvement_service)
    ...
    def hide_to_tray(self) -> None:
        ...
        if self.status_label:
            self.status_label.config(text="アプリをトレイに格納しました")
    ...
    def create_widgets(self) -> None:
        ...
        improvement_button = tk.Button(
            self.root,
            text="要望",
            command=self.improvement_dialog.show_management_window,
    ...
    def show_task_selection_dialog(self) -> None:
        ...
        dialog.protocol("WM_DELETE_WINDOW", self.hide_to_tray)
